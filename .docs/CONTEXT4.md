...
_Avoid_: Embedding Training Run, Transformer Training Run
...
_Avoid_: Embedding Epoch Update, Transformer Epoch Update
...
_Avoid_: Training Run when referring to Word2Vec, Transformer Training Run
...
**Transformer Training Corpus**:
The fixed ordered collection of stories used to learn the Transformer-specific BPE model and construct Transformer Training Sequences. Client input and Generated Text Samples never alter this corpus.
_Avoid_: Embedding Training Corpus, user prompt, seed text

**Transformer Training Sequence**:
One fixed-length ordered sequence of token IDs paired with its next-token target IDs for decoder-only Transformer training.

**Transformer Preprocessing Snapshot**:
The immutable application-wide result of processing the Transformer Training Corpus, including the Merge Table, tokenized stories, ordered Vocabulary, token indices, complete token-ID sequence, Transformer Training Sequences, generation seed IDs, and four Logical Training Shard boundaries. It is initialized lazily and reused by every Transformer Training Run.
...
_Avoid_: Global random stream, shared sampler state

**Transformer Epoch Update**:
A streamed Transformer progress measurement containing the current epoch, its six-decimal training loss, and one Generated Text Sample. A Transformer Training Run emits approximately fifty Transformer Epoch Updates, including epoch zero and the exact requested final epoch.
_Avoid_: Epoch Update when the Learning Demo is ambiguous
...
**Ordered Gradient Reduction**:
The deterministic combination of the four Logical Training Shard losses and gradients in canonical order `0 → 1 → 2 → 3` before one optimizer update. Worker completion order does not change the reduction order.
...
_Avoid_: Global shared-memory cache, cross-request shared weights
...
**Transformer Training Compatibility**:
The requirement that the Python Transformer preserve the reference decoder-only architecture, corpus-derived BPE behavior, request and SSE payload shapes, `float32` stored numerical state, seeded behavior, and educational outcome while using stable Python-owned rules for progress reporting, process partitioning, gradient reduction, sampling streams, caching, persistence, cleanup, and failure handling.
_Avoid_: Exact TypeScript worker parity, host-dependent equivalence
...
The original Hono and TypeScript server code consulted only to determine behavior that the Python Backend must reproduce or deliberately stabilize. It is not stored, executed, or retained as part of the current backend.
